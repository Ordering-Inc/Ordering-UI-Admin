...
var SingleBusinessProduct = function SingleBusinessProduct(props) {
  ...
  var _useState5 = (0, _react.useState)(null),
      ...
      subCategoryToEdit = _useState6[0],
      setSubCategoryToEdit = _useState6[1];
  /**
  ...
  */
  ...
  var categoriesToEdit = [];
  var categoryA = null;
  var categoryParent = null;
  var indexAux = 0;

  var iterateCategories = function iterateCategories(categories, params, index) {
    categories && categories.forEach(function (item, i) {
      if (item.id === (categorySelected === null || categorySelected === void 0 ? void 0 : categorySelected.id)) {
        var _products = categorySelected.products.map(function (prod) {
        ...
        categoryA = {
          category: _objectSpread(_objectSpread({}, categorySelected), {}, {
            products: _products
          }),
          level: index + 1,
          index: indexAux
      ...
      if (!categoryA) {
        iterateCategories(item.subcategories, params, index + 1);
      }
    }); // return { categoryNew: categoryA, index: i }
  };

  var iterator = 0;

  var replaceInfo = function replaceInfo(subcategory, _categorySelected, index) {
    // console.log(subcategories, _categorySelected, index)
    // console.log(subcategories)
    if (iterator === 5) return;
    if (!subcategory) return;
    var yanose = subcategory && subcategory.subcategories.map(function (subcategory2) {
      // if (subcategory2.id === _categorySelected.id && iterator === index) {
      //   return {
      //     ...subcategory2,
      //     ..._categorySelected
      //   }
      // }
      if (iterator === index) {
        return _objectSpread(_objectSpread({}, subcategory2), _categorySelected);
      } else {
        return subcategory2;
      }
    });
    ...
    replaceInfo(subcategory, _categorySelected, index);
  };
  /**
  ...
  var editProduct = /*#__PURE__*/function () {
    var _ref = _asyncToGenerator( /*#__PURE__*/_regenerator.default.mark(function _callee(params) {
      var _yield$ordering$busin, _yield$ordering$busin2, error, result, itemFounded, _categories, algo;
            ...
            case 7:
              ...
              if (!error) {
                ...
                if (handleUpdateBusinessState) {
                  ...
                  if (!itemFounded) {
                    iterateCategories(_categories, params, 0);
                    algo = replaceInfo(_categories[categoryA.index - 1], categoryA.category, categoryA.level);
                  }
                ...
                showToast(_orderingComponentsAdmin.ToastType.Success, t('PRODUCT_UPDATED', 'Product updated'));
                setIsEditMode(false);
              } else {
                setFormState(_objectSpread(_objectSpread({}, formState), {}, {
                  loading: false,
