...
export const BusinessAnalytics = (props) => {
  ...
  const [filterList, setFilterList] = useState({ lapse: 'today', businessIds: null, app_id: 'all', franchises_id: null, countryIds: null })
  ...
  const paramsForAPI = (type) => {
    ...
    if (filterList?.countryIds && filterList?.countryIds.length > 0) params = `${params}&countries=${filterList?.countryIds?.toString()}`
